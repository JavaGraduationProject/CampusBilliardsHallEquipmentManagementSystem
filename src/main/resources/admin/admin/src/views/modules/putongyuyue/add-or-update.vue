...
export default {
  ...
  computed: {
    ...
    zongjine:{
      get: function () {
        return 1*this.ruleForm.putongshijia*this.ruleForm.yuyueshizhang
      }
    },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `putongyuyue/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.putongyuyueCrossAddOrUpdateFlag = false;
						 this.parent.search();
